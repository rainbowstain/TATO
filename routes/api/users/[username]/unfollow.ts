...
export const handler: Handlers = {
  async POST(req, ctx) {
    try {
      ...
      // No permitir dejar de seguirse a sí mismo
      if (currentUsername === targetUsername) {
        return new Response(
          JSON.stringify({ error: "No puedes dejar de seguirte a ti mismo" }),
          {
            status: 400,
            headers: { "Content-Type": "application/json" },
          },
      ...
      if (!users[currentUserIndex].following.includes(targetUsername)) {
        return new Response(
          JSON.stringify({ error: "No estás siguiendo a este usuario" }),
          {
            status: 400,
            headers: { "Content-Type": "application/json" },
          },
      ...
      // Eliminar de la lista de seguidos del usuario actual
      users[currentUserIndex].following = users[currentUserIndex].following
        .filter(
          (username) => username !== targetUsername,
        );

      // Eliminar de la lista de seguidores del usuario objetivo
      users[targetUserIndex].followers = users[targetUserIndex].followers
        .filter(
